...
if( empty( $options['hosts']['wistia']['authors'] ) ) return;

// Classname should not conflict with default MEXP Services
// in case MEXP is already installed
class MEXP_EV_Wistia_Service extends MEXP_Service {

  const SERVICE = 'wistia';
  const EV_SERVICE = 'ev_wistia';

  public function __construct() {

    require_once dirname( __FILE__ ) . '/mexp-ev-wistia-template.php';
    # Go!
    $this->set_template( new MEXP_EV_Wistia_Template );
  }
  ...
  public function labels( array $labels ) {
    ...
    $labels[self::EV_SERVICE] = array(
      'title'     => __( 'Insert Wistia EV', 'mexp' ),
...
add_filter( 'mexp_services', 'mexp_service_ev_wistia' );

function mexp_service_ev_wistia( array $services ) {
  $services['ev_wistia'] = new MEXP_EV_Wistia_Service;
  return $services;
}
