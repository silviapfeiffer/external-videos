...
if( empty( $options['hosts']['dailymotion']['authors'] ) ) return;

// Classname should not conflict with default MEXP Services
// in case MEXP is already installed
class MEXP_EV_Dailymotion_Service extends MEXP_Service {

  const SERVICE = 'dailymotion';
  const EV_SERVICE = 'ev_dailymotion';

	public function __construct() {

    require_once dirname( __FILE__ ) . '/mexp-ev-dailymotion-template.php';
		# Go!
		$this->set_template( new MEXP_EV_Dailymotion_Template );
	}
  ...
  // EV media explorer tabs are by author
	...
	public function labels( array $labels ) {
		...
		$labels[self::EV_SERVICE] = array(
			'title'     => __( 'Insert Dailymotion EV', 'mexp' ),
...
add_filter( 'mexp_services', 'mexp_service_ev_dailymotion' );

function mexp_service_ev_dailymotion( array $services ) {
	$services['ev_dailymotion'] = new MEXP_EV_Dailymotion_Service;
	return $services;
}
