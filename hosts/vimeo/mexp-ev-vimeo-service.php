...
if( empty( $options['hosts']['vimeo']['authors'] ) ) return;

// Classname should not conflict with default MEXP Services
// in case MEXP is already installed
class MEXP_EV_Vimeo_Service extends MEXP_Service {

  const SERVICE = 'vimeo';
  const EV_SERVICE = 'ev_vimeo';

  public function __construct() {

    require_once dirname( __FILE__ ) . '/mexp-ev-vimeo-template.php';
    # Go!
    $this->set_template( new MEXP_EV_Vimeo_Template );
  }
...
add_filter( 'mexp_services', 'mexp_service_ev_vimeo' );

function mexp_service_ev_vimeo( array $services ) {
  $services['ev_vimeo'] = new MEXP_EV_Vimeo_Service;
  return $services;
}
